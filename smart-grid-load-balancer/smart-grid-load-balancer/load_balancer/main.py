...
# Lock for thread-safe access to load data
        ...
        try:
            for substation in SUBSTATIONS:
                try:
                    ...
                    # Keep the last known load value
...
def get_least_loaded_substation():
    ...
    with load_lock:
        if not substation_loads:
            return SUBSTATIONS[0]  # Default to first substation
...
def route_charge():
    ...
    try:
        ...
        # Forward the request to the selected substation
